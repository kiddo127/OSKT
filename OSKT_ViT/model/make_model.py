...
class build_transformer_pass(nn.Module):
    def __init__(self, num_classes, camera_num, cfg, backbone_embed_dim, factory):
    ...
    def load_param(self, trained_path):
        ...
        for i in param_dict:
            try:
                # self.state_dict()['base.'+i.replace('module.', '')].copy_(param_dict[i])
                j = i.replace('module.', '')
                if 'bottleneck' not in i and 'classifier' not in i:
                    if 'base' not in j:
                        j = 'base.'+j 
...
def make_model(cfg, num_class, camera_num, backbone_embed_dim=None):
    if cfg.MODEL.NAME == 'transformer':
        if cfg.MODEL.JPM:
            model = build_transformer_local(num_class, camera_num, cfg, __factory_T_type, rearrange=cfg.MODEL.RE_ARRANGE)
            print('===========building transformer with JPM module ===========')
        else:
            model = build_transformer_pass(num_class, camera_num, cfg, backbone_embed_dim, __factory_T_type)
