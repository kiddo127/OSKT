...
from .triplet_loss import TripletLoss, BatchSoftTripletLoss


def make_loss(cfg, num_classes, feat_dim, center_loss_only=False):
    feat_dim = feat_dim
    ...
    if 'triplet' in cfg.LOSS_TYPE:
        if cfg.use_softTrip:
            triplet = BatchSoftTripletLoss(cfg.MARGIN, cfg.HARD_FACTOR)  # triplet loss
        else:
    ...
    def loss_func(score, feat, target, use_center=True):
